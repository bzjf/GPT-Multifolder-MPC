...
- 支持查看 Node.js 端口占用
- 支持通过弹窗选择文件夹
- 支持在网页中直接初始化本机 MCP、查看工具并手动调用，无需经过 ChatGPT 或 Funnel
...
后台允许 Funnel 之后，可以在本项目控制面板中单独开启或关闭 Funnel。MCP 实例和 Funnel 是两个独立状态：关闭 Funnel 只会切断 ChatGPT 使用的公网入口，不会停止本机正在运行的 MCP 服务。
...
在实例列表中找到刚添加的仓库，点击“启动”。控制面板会先显示“启动中”，并有界轮询实例的 `/health`；只有健康检查成功后才显示“MCP：已就绪”并返回启动成功。

然后在控制面板的“Funnel 访问控制”区域点击“开启 Funnel”。实例列表会分别显示 MCP 和 Funnel 状态；只有两者都正常时，整体状态才会显示“连接可用”，并出现可复制的 URL。

### 本机手动测试（可选，不需要 Funnel）

实例启动后，使用页面底部的“本机 MCP 手动测试”区域：

1. 选择一个“已就绪”的实例，点击“重新初始化”或直接点击“读取工具”。
2. 在工具下拉框中选择要调用的 MCP 工具。
3. 按工具的 inputSchema 在参数框中填写 JSON 对象，然后点击“调用工具”。
4. 在响应框查看结构化 MCP 调用结果和当前 session/协议连接信息。

这条链路是“网页 → 控制面板同源 API → 127.0.0.1 上的 MCP 实例”。它模拟 ChatGPT 的 MCP 客户端行为，但由你手动选择工具和填写参数。非只读工具会再次确认，并继续受实例的 read / write / ship 权限与工具参数约束。
...
如果添加失败，先检查控制面板里的整体状态是否是“连接可用”，再分别查看 MCP 是否运行、Funnel 是否开启。
...
所有自动化测试、测试夹具和手动测试案例都集中在 [`gpt-repo-mcp/tests/`](gpt-repo-mcp/tests/README.md)：

- `server/`：MCP server、工具契约、读写策略和 Git 工作流的 Vitest 测试。
- `control-panel/`：控制面板、本机 MCP 客户端和实例生命周期的 Node 测试。
- `manual/`：可以直接复制到网页控制面板的手动调用案例。

仓库中的 `scripts/` 只保留运行实现，不再混放测试文件。
...
因此，不要把 URL 发到公开聊天、公开仓库、截图、日志或文档里。暂时不使用时，可以直接在控制面板中关闭 Funnel，MCP 服务器仍可继续在本机运行；长期不用时，再停止实例并删除 ChatGPT Connector 配置。
...
连接成功后，ChatGPT 会通过 MCP 工具接口访问仓库。这里不逐个列出全部工具名，只按能力分组说明。

| 能力层级 | 说明 | 典型工具 |
|---|---|---|
| 阅读与分析 | 查看仓库、搜索文件、读取内容、查看 diff、理解项目结构和当前状态 | repo_tree、repo_search、repo_fetch_file、repo_git_diff、repo_project_brief |
| 图片读取 | 读取仓库内指定的 PNG、JPEG 或 WebP 图片，用于分析截图、界面图、流程图和设计图 | repo_fetch_image |
| 大文本分页 | 对超大 UTF-8 文本按行范围、字节偏移或 cursor 分页读取，单次返回仍受字节上限约束 | repo_fetch_file |
| 修改文件 | 在用户授权范围内修改本地文件，或写入给 Codex 使用的任务文件 | repo_write_file、repo_write_changes、repo_write_codex_task |
| 工作区整理 | 暂存、取消暂存、创建本地提交、恢复错误修改、清理生成文件 | repo_write_stage、repo_write_commit、repo_write_recover、repo_cleanup_paths |
| Codex 技能与任务 | 列出并读取本机 Codex skills，生成 Codex 任务，读取并复核 Codex 执行结果 | codex_list_skills、codex_read_skill、repo_write_codex_task、repo_codex_review |
| 权限与安全说明 | 解释某个路径为什么能读、能写或被拒绝，帮助排查权限和安全拦截问题 | repo_policy_explain、repo_last_write、repo_git_review |

图片读取必须指定仓库内的具体文件路径，并受大小、路径和文件类型限制。图片像素中的账号、密钥或个人信息无法像文本那样自动扫描，调用前应先确认图片内容适合发送给 ChatGPT。

大文本文件不会再因为整个文件超过单次上限而直接拒绝。未指定范围时返回第一页并给出 next_cursor；指定 start_line/end_line 时流式扫描目标行；单行超长文件可以使用 byte_offset 或 cursor 继续读取。行模式默认最多扫描 64 MiB 源文件内容，可通过 limits.max_line_scan_bytes 调整；超过后应改用字节分页，避免一次调用长时间扫描超大文件。字节分页还会扫描页面前后的上下文，防止疑似敏感值恰好被分页边界拆开；只有匹配内容跨越页面边界时才会拒绝，完整落在页面内的值仍按普通文本规则打码。

不同权限模式对应不同能力边界。

| 模式 | 开放能力 | 适合场景 |
|---|---|---|
| read | 阅读与分析、图片读取、大文本分页、权限说明、Codex skill 读取 | 让 ChatGPT 看项目、解释代码、检查问题 |
| write | read 的能力，加上修改文件 | 让 ChatGPT 修改 README、配置、源码或生成 Codex 任务 |
| ship | write 的能力，加上工作区整理和本地提交 | 提交前整理、暂存、创建本地 commit、恢复错误改动 |

建议默认使用 read。需要改文件时再切到 write。ship 只在准备提交或整理工作区时短时间使用。
...
注意这里分两层：Tailscale 管理后台负责允许 Funnel；本项目控制面板负责单独开启或关闭 Funnel，并在 Funnel 开启后提供可访问的 URL。后台没有允许 Funnel 时，控制面板无法单独完成这一步。
