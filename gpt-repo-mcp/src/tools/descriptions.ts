export const descriptions = {
  ...
  codex_list_skills:
    "Use this when the user asks which local Codex skills are installed. Returns SKILL.md frontmatter metadata only and does not read skill body content or arbitrary paths.",
  codex_read_skill:
  ...
  repo_edit_context:
    "Use this when the user asks to edit, fix, debug, or implement code and likely files are not fully known. Batches compatible queries into one repository scan, ranks candidates, returns a compact first page of file contents, and includes git HEAD so the next step can usually be repo_write_changes.",
  repo_fetch_file:
