...
import { CodexSkillsService } from "../services/codex-skills-service.js";
...
export const codexListSkillsHandler: ToolHandler = async (input, context) => safeTool<CodexSkillsInput>("codex_list_skills", input, context, async (args) => {
  const result = await new CodexSkillsService().list(args);
  audit({ tool: "codex_list_skills", counts: { skills: result.returned_count }, truncated: result.truncated, warnings: result.warnings });
  return createSuccessEnvelope(result, `Returned ${result.returned_count} Codex skills.`, { warnings: result.warnings });
});

export const codexReadSkillHandler: ToolHandler = async (input, context) => safeTool<CodexReadSkillInput>("codex_read_skill", input, context, async (args) => {
  const result = await new CodexSkillsService().read(args);
  audit({ tool: "codex_read_skill", counts: { bytes: result.size_bytes }, truncated: result.truncated, warnings: result.warnings });
  return createSuccessEnvelope(result, `Read Codex skill ${result.skill.name}.`, { warnings: result.warnings });
});
