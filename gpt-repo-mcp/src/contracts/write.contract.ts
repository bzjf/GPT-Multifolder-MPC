...
export const WriteChangesInputSchema = RepoInputSchema.extend({
  changes: z.array(WriteChangeSchema).min(1).max(25).describe("Ordered edit pack of one-file write or edit changes. Each repo-relative target path may appear at most once; when one file needs multiple edits, combine them into one type=edit change with an ordered edits array. All changes are prepared and validated before writes begin; successful preparations are then committed in request order. No git stage or commit is performed."),
