...
describe("FileReader", () => {
  ...
  test("does not expose a trailing newline as an extra editable line", async () => {
    const fixture = await createRepoFixture();
    const reader = new FileReader(new PathSandbox(fixture.root));

    await expect(reader.read({ path: "src/app.ts", start_line: 4, end_line: 4 })).rejects.toMatchObject({
      code: "VALIDATION_ERROR"
    });
  });

  test("reports an empty file as zero lines", async () => {
    const fixture = await createRepoFixture();
    await writeFile(join(fixture.root, "docs", "empty.txt"), "");
    const reader = new FileReader(new PathSandbox(fixture.root));

    const result = await reader.read({ path: "docs/empty.txt" });

    expect(result.total_lines).toBe(0);
    expect(result.start_line).toBeUndefined();
    expect(result.end_line).toBeUndefined();
    await expect(reader.read({ path: "docs/empty.txt", start_line: 1 })).rejects.toMatchObject({
      code: "VALIDATION_ERROR"
    });
  });
