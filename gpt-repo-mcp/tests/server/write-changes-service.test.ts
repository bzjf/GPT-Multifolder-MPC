...
describe("WriteChangesService", () => {
  ...
  test("grouped line edits keep original 1-based coordinates after earlier insertions", async () => {
    const fixture = await createRepoFixture();
    const service = createService(fixture.root, { enabled: true, allowed_globs: ["src/**"] });
    await writeFile(join(fixture.root, "src", "app.ts"), "one\ntwo\nthree\nfour\n");
      ...
      changes: [
        {
          ...
          path: "src/app.ts",
          edits: [
            { type: "insert_before_line", start_line: 2, content: "inserted\n" },
            { type: "replace_lines", start_line: 4, end_line: 4, content: "FOUR\n" }
          ]
        }
      ]
    });

    await expect(readFile(join(fixture.root, "src", "app.ts"), "utf8")).resolves.toBe("one\ninserted\ntwo\nthree\nFOUR\n");
  });

  test("grouped edits reject mixed and overlapping coordinate systems", async () => {
    const fixture = await createRepoFixture();
    const service = createService(fixture.root, { enabled: true, allowed_globs: ["src/**"] });
    const original = await readFile(join(fixture.root, "src", "app.ts"), "utf8");
    ...
    await expect(service.apply({
      changes: [{
        ...
        path: "src/app.ts",
        edits: [
          { type: "replace_lines", start_line: 1, end_line: 1, content: "changed\n" },
          { type: "replace", find: "rawFetch", replace: "safeFetch" }
        ]
      }]
    ...
    await expect(service.apply({
      changes: [{
        ...
        path: "src/app.ts",
        edits: [
          { type: "replace_lines", start_line: 1, end_line: 2, content: "changed\n" },
          { type: "insert_before_line", start_line: 2, content: "overlap\n" }
        ]
      }]
    })).rejects.toMatchObject({ code: "VALIDATION_ERROR" });

    await expect(readFile(join(fixture.root, "src", "app.ts"), "utf8")).resolves.toBe(original);
  });
