...
describe("FileWriter", () => {
  ...
  test("uses the exact final 1-based line reported by FileReader", async () => {
    const fixture = await createRepoFixture();
    const sandbox = new PathSandbox(fixture.root);
    const reader = new FileReader(sandbox);
    const writer = new FileWriter(fixture.root, sandbox, new WritePolicy({ enabled: true, allowed_globs: ["src/**"] }));
    const snapshot = await reader.read({ path: "src/app.ts" });
    const finalLine = snapshot.end_line;
    if (finalLine === undefined) throw new Error("Expected a non-empty fixture file.");

    await writer.write({
      path: "src/app.ts",
      action: "replace_lines",
      start_line: finalLine,
      end_line: finalLine,
      content: "} // final line\n"
    });

    expect(finalLine).toBe(3);
