...
describe("package startup scripts", () => {
  test("keeps every automated test under the canonical tests directory", async () => {
    const repositoryRoot = join(process.cwd(), "..");
    const testFiles = await findTestFiles(repositoryRoot);

    expect(testFiles.length).toBeGreaterThan(0);
    expect(testFiles.every((path) => path.startsWith("gpt-repo-mcp/tests/"))).toBe(true);
    expect(testFiles.some((path) => path.startsWith("gpt-repo-mcp/tests/server/"))).toBe(true);
    expect(testFiles.some((path) => path.startsWith("gpt-repo-mcp/tests/control-panel/"))).toBe(true);
  });
  ...
  test("declares local startup shortcuts", async () => {
    ...
    expect(pkg.scripts?.["lint:control-panel"]).toBe("node ../scripts/lint-control-panel.mjs");
    expect(pkg.scripts?.test).toBe("vitest run");
    expect(pkg.scripts?.["test:control-panel"]).toBe("node --test tests/control-panel/test-all.mjs");
    expect(pkg.scripts?.["test:smoke"]).toBe("vitest run tests/server/mcp-contract.test.ts tests/server/tool-contracts.test.ts tests/server/file-reader.test.ts");
    expect(pkg.scripts?.verify).toBe("npm run typecheck && npm run build && npm run test:control-panel && npm run test:smoke");
    expect(pkg.scripts?.["verify:full"]).toBe("npm run typecheck && npm run build && npm run test:control-panel && npm test");
  });
...
const TEST_SCAN_EXCLUDES = new Set([
  ".git",
  ".runtime",
  "dist",
...
async function findTestFiles(root: string, directory = root): Promise<string[]> {
  const found: string[] = [];
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    if (entry.isDirectory()) {
      if (!TEST_SCAN_EXCLUDES.has(entry.name)) {
        found.push(...await findTestFiles(root, join(directory, entry.name)));
      }
      continue;
    }
...
function isTestFile(name: string): boolean {
  return name.endsWith(".test.ts") || name.endsWith(".test.mjs") || name === "test-all.mjs";
}
