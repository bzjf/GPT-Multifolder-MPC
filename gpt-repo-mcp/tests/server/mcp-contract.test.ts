...
describe("MCP contract", () => {
  test("initialize exposes server instructions and tool capability", async () => {
    ...
    try {
      ...
      expect(SERVER_INSTRUCTIONS).toContain("CRLF/LF");
      expect(SERVER_INSTRUCTIONS).toContain("replace_lines");
  ...
  test("tools/list exposes schemas and appropriate annotations for every tool", async () => {
    ...
    try {
      ...
      const writeFile = listed.tools.find((tool) => tool.name === "repo_write_file");
      const actionSchema = writeFile?.inputSchema.properties?.action as {
        enum?: unknown[];
        description?: string;
      } | undefined;
      expect(actionSchema?.enum?.slice(0, 3)).toEqual([
        "replace_lines",
        "insert_before_line",
        "insert_after_line"
      ]);
      ...
      expect(listed.tools.map((tool) => ({
        ...
        [
          ...
          },
          {
            ...
            "description": "Use this when the user asks to find code, inspect usages, perform a bughunt, or locate relevant files before reading them. Result line values are exact 1-based source coordinates and should be copied unchanged into line-number write actions. Uses a bounded ripgrep fast path when available and a safe TypeScript fallback; matched_count is a lower bound when scan_complete is false. Prefer this before repo_read_many.",
            "inputKeys": [
              "context_lines",
              "cursor",
              "exclude_globs",
          ...
          },
          {
            ...
            "description": "Use this when the user names exactly one specific text file or needs a narrow line/byte page from one file. Returned start_line/end_line are exact 1-based inclusive source coordinates; a trailing newline is not an extra editable line. Copy these coordinates unchanged into line-number write actions. After search finds multiple likely files, prefer repo_read_many or repo_edit_context instead of repeated repo_fetch_file calls.",
            "inputKeys": [
              "byte_offset",
              "cursor",
              "end_line",
...
function representativeCalls(head: string): Record<string, Record<string, unknown>> {
  ...
  repo_edit_context: { repo_id: "fixture", goal: "Update fixture app", search_queries: ["fixture"], known_paths: ["src/app.ts"], max_files_to_read: 2 },
  ...
  repo_prepare_codex_task: {
    ...
    verification_commands: ["npm test -- tests/server/mcp-contract.test.ts"]
  },
