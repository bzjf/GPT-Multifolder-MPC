...
describe("ChangePlanService", () => {
  ...
  test("deep planning uses the elevated 60-file inspection capacity", async () => {
    const fixture = await createRepoFixture();
    await mkdir(join(fixture.root, "many"), { recursive: true });
    await Promise.all(Array.from({ length: 70 }, (_, index) =>
      writeFile(join(fixture.root, "many", `capacity-${index}.ts`), "export const capacity = true;\n")
    ));

    const result = await new ChangePlanService(fixture.root, new PathSandbox(fixture.root)).plan({
      goal: "Update capacity modules",
      include_globs: ["many/**/*.ts"],
      planning_depth: "deep"
    });

    expect(result.relevant_files).toHaveLength(60);
    expect(result.warnings).toContain("RELEVANT_FILE_LIMIT_REACHED");
  });
