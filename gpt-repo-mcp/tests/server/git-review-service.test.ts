...
describe("GitReviewService", () => {
  ...
  test("compact review caps summaries and skips an absent staged diff", async () => {
    ...
    const statusLines = Array.from({ length: 120 }, (_, index) =>
      `1 .M N... 100644 100644 100644 ${"1".repeat(40)} ${"1".repeat(40)} docs/file-${index}.md`
    );
    const diff = Array.from({ length: 120 }, (_, index) => [
    ...
    expect(result.diff_summary.file_count).toBe(120);
    expect(result.diff_summary.files).toHaveLength(100);
